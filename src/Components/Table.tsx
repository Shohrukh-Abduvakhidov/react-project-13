...
const Table = ({
	...
	setEdit: (edit: IUser | null) => void
	emailE: string
	setEmailE: (emailE: string) => void
	openE: boolean
	setOpenE: (openE: boolean) => void
}) => {
	function deleteUser(id: number | string | undefined) {
    if(id !== undefined)
    {
	...
	function checked(id: string | number | undefined) {
		if (id !== undefined) {
			setUSers(
				users.map(user =>
	...
	function EditForm(e: React.FormEvent) {
		...
		setUSers(
			users.map((user: IUser) => (user.id === edit.id ? updateUser : user))
		)
		setOpenE(false)
	}
